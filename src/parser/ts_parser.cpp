...
void TSParserWrapper::extract_functions(TSNode node, const std::string& source,
    ...
    if (std::strcmp(type, "decorated_definition") == 0) {
        ...
        for (uint32_t i = 0; i < count; i++) {
            ...
            if (std::strcmp(child_type, profile.function_def_type().c_str()) == 0) {
                ...
                // Decorated class — recurse to handle via class extraction (commit 6)
                extract_functions(child, source, file_path, profile, results, parent_class);
            }
        }
        return;
    }
    ...
    // Recurse into children for top-level/class body traversal
    uint32_t count = ts_node_child_count(node);
    for (uint32_t i = 0; i < count; i++) {
